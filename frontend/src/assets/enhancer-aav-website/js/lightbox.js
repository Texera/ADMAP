...
import {
  ...
  metaZoom,
  biologicalMetadata,
  biologicalMetadataTitle,
  metaMouseId,
  metaGenotype,
  metaEnhancer,
  metaVirus,
  metaVirusLabel,
  metaValidationMethod,
  metaInfection,
  metaMOI,
  metaIncubation,
  metaMouseIdRow,
  ...
  metaEnhancerRow,
  metaVirusRow,
  metaTargetedCellsRow,
  metaValidationMethodRow,
  metaInfectionRow,
  metaMOIRow,
  metaIncubationRow,
  redSlider,
...
import { channelState, viewState, lightboxState, zoomState, touchState, regionsOverlayState } from "./state.js";
...
export function updateMetadata(imageData) {
  metaDimensions.textContent = imageData.width && imageData.height ? `${imageData.width} × ${imageData.height}` : "—";
  ...
  if (imageData.metadata && Object.keys(imageData.metadata).length > 0) {
    ...
    const isInVitro = metadata["Validation Method"] && metadata["Validation Method"].toLowerCase().includes("in vitro");
...
export function openAt(i) {
  ...
  if (!lbIsInVitro && window.innerWidth > MOBILE_BREAKPOINT) {
    ...
    updateAtlasPanel(imageData.ccf_index !== undefined ? imageData.ccf_index : null, imageData.registered || null);
  } else {
  ...
  if (regionsOverlayToggle) {
    ...
    regionsOverlayToggle.style.display = hasInverseAnnotation && !lbIsInVitro ? "inline-block" : "none";
  }
  ...
  const preloadAdjacentFn = () => {
    for (const adjIdx of [prevIdx, nextIdx]) {
      if (lightboxState.items[adjIdx]) {
        const adjBtn =
          lightboxState.items[adjIdx].tagName === "FIGURE"
...
export function initLightbox() {
  ...
  if (ccfSliceImage) {
    ccfSliceImage.addEventListener("click", e => {
      e.stopPropagation();
      showInteractiveAtlas();
    });
  }
  if (atlasBrainImage) {
    atlasBrainImage.style.cursor = "pointer";
    atlasBrainImage.addEventListener("click", e => {
      e.stopPropagation();
      showInteractiveAtlas();
    });
  }
  ...
  if (atlasBackBtnEl) {
    atlasBackBtnEl.addEventListener("click", e => {
      e.stopPropagation();
      showCcfSliceView();
    });
  }
  ...
  if (metadataPanel) {
    ...
    metadataPanel.addEventListener(
      "touchstart",
      e => {
